...
class Plist {
  ...
  parseBinary( buffer, offset, length ) {
    this.data = Plist.binary.decode( buffer, offset, length )
    return this
  }
  ...
  parseXML( buffer, offset, length ) {
    this.data = Plist.xml.decode( buffer, offset, length )
    return this
  }
   ...
   */
  toString() {
    return Plist.xml.encode( this.data )
  }
  ...
  write( buffer, offset, encoding ) {
    ...
    switch( encoding ) {
      case 'binary': return Plist.binary.encode( this.data, buffer, offset )
...
Plist.xml = require( './xml' )

Plist.binary = require( './binary' )
