...
function readInt( context, buffer, offset ) {
  ...
  switch( length ) {
    ...
    case 8: return Number( buffer.readBigInt64BE( context.offset + offset + 1 ) )
    case 16: return Number(
      ( buffer.readBigInt64BE( context.offset + offset + 1 ) << 64n ) +
      buffer.readBigUInt64BE( context.offset + offset + 1 + 8 )
    )
    default:
...
function decode( buffer, offset, length ) {
  ...
  var context = {
    ...
    objectCount: Number( buffer.readBigUInt64BE( trailerOffset + 8 ) ),
    rootObject: Number( buffer.readBigUInt64BE( trailerOffset + 16 ) ),
    offsetTableOffset: Number( buffer.readBigUInt64BE( trailerOffset + 24 ) ),
    // Offset Table
    offsetTable: null,
  }
